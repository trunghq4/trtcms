					...
					<table id="datatable" class="table table-bordered table-striped jambo_table bulk_action">
						...
						@foreach($list_news as $items)
							<tr class="list_news">
								...
								<td><input type="checkbox" name="publish" id_news="{{$items->id}}" @if($items->publish == 1) checked @endif title="Xuất bản"></td>
								...
								<input type="checkbox" name="home" class="col-md-2 col-xs-2 col-sm-2" id_news="{{$items->id}}" @if($items->home == 1) checked @endif title="Trang chủ">
								<div class="clearfix"></div>
								<label class="label-primary col-md-10 col-xs-10 col-sm-10" style="color: #fff">Nổi bật</label>
								<input type="checkbox" name="hot" class="col-md-2 col-xs-2 col-sm-2" id_news="{{$items->id}}" title="Nổi bật" @if($items->hot == 1) checked @endif>
								<div class="clearfix"></div>
								<label class="label-warning col-md-10 col-xs-10 col-sm-10" style="color: #fff">Tiêu điểm</label>
								<input type="checkbox" name="focus" class="col-md-2 col-xs-2 col-sm-2" id_news="{{$items->id}}" title="Tiêu điểm" @if($items->focus == 1) checked @endif></div></td>
								@endif
								<div class="clearfix"></div>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('input[name=publish]').click(function(){
			url = "{{url('admin/ajax/news_publish')}}/";
	...
	// 			url = '{{url("admin/ajax/list-news/")}}/'+count;
	// 			// console.log(count);
	// 			$.ajax({
	// 				url:url,
	// 				type:'get',
