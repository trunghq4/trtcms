...
<div class="">
  <div class="form-group">
    @if(Session::get('user')->add_product_cate == 1) <a class="btn btn-success" href="{{url('admin/add-country-product')}}">Thêm hãng</a> @endif
      ...
      <div class="x_panel">
        ...
        <div class="x_content table-responsive">
            <table id="datatable" class="table table-bordered table-striped jambo_table bulk_action">
              ...
              <tbody>
              @foreach($list_country as $items)
              @if($items->id > 1)
                <tr>
                  <td><input type="checkbox" class="check_del flat" name="check_del[]" value="{{$items->id}}"></td>
                  <td>{{$items->id}}</td>
                  <td><strong>{{$items->name}}</strong></td>
                  @if(Session::get('user')->edit_product_cate == 1) <td><a href="{{domain}}admin/edit-country-product/{{$items->id}}" class="btn btn-warning btn-xs"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i></a> <a href="{{domain}}admin/del-country-product/{{$items->id}}" onclick="return confirm('Bạn chắc chắn muốn xóa!')" class="btn btn-danger btn-xs"><i class="fa fa-times" aria-hidden="true"></i></a></td> @endif
                </tr>
                @endif
                @endforeach
              </tbody>
